...
def train_model():
    ...
    model.train(
        ...
        epochs=100,              # Giảm số epoch để tránh overfitting
        batch=8,                 # Giảm batch size để ổn định hơn
        name="xiangqi_model",    # Tên thư mục output
        workers=4,               # Số lượng luồng để load data
        patience=20,             # Điều chỉnh patience
        optimizer='SGD',         # Dùng SGD optimizer cho ổn định
        lr0=0.01,               # Learning rate khởi đầu cao hơn
        lrf=0.001,              # Learning rate cuối cao hơn
        weight_decay=0.0005,     # Giữ nguyên regularization
        warmup_epochs=3,         # Giảm warmup epochs
        box=5.0,                 # Giảm hệ số loss box
        cls=0.5,                 # Tăng class loss weight
        hsv_h=0.3,              # Tăng augmentation hue
        hsv_s=0.7,              # Tăng saturation augmentation
        hsv_v=0.4,              # Giữ nguyên value augmentation
        degrees=10.0,           # Tăng góc xoay
        translate=0.2,          # Tăng mức dịch chuyển
        scale=0.3,              # Giảm scale augmentation
        shear=0.1,              # Thêm shear nhẹ
        perspective=0.0,        # Vẫn giữ không perspective
        flipud=0.0,             # Vẫn không lật dọc
        fliplr=0.3,             # Giảm tỉ lệ lật ngang
        mosaic=0.8,             # Giảm nhẹ mosaic
        mixup=0.0,              # Tắt mixup
        amp=True,               # Giữ mixed precision
